defmodule DuckFeeder.Integration do
  ...
  def runtime_child_spec_from_config(meta_conn, config, opts \\ []) do
    runtime_config =
      %{config: config}
      |> maybe_put_source_name(Keyword.get(opts, :source_name))

    with {:ok, resolved} <- Runtime.resolve_app_config(runtime_config) do
      runtime_opts =
  ...
  defp maybe_put_source_name(config, nil), do: config
  defp maybe_put_source_name(config, source_name), do: Map.put(config, :source_name, source_name)
end
