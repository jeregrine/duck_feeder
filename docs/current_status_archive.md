# DuckFeeder Status Archive (Detailed)

Historical detailed status/checklist snapshot archived from `docs/current_status.md`
during compaction. The active source-of-truth task board is now `docs/current_status.md`.

## Priority TODO (pre-production, pre-compaction context)

- [ ] **P0 data-integrity: WAL ack safety**
  - move replication ack advancement from commit-decode time to durable-commit/checkpoint time
  - add explicit service→connection ack contract and crash-injection tests proving no ack-before-commit loss window
- [ ] **P0 data-integrity: bootstrap/start LSN correctness for existing slots**
  - avoid `pg_current_wal_lsn` skip-forward behavior when slot already exists
  - derive safe start point from slot/checkpoint semantics and add coverage for backlog-preserving restart cases
- [ ] **P0 recovery: reconcile `:encoded` batches by default**
  - include `:encoded` in stale-state reconciliation policy (or collapse state transitions to remove stranded intermediate state)
  - add integration for crash window between upload and `:uploaded` transition
- [ ] **P1 runtime safety: bounded ingest/backpressure**
  - stop synchronous long-running batch work from blocking `DuckFeeder.Service` mailbox
  - introduce bounded in-flight processing/backpressure policy and telemetry for queue depth/latency
- [ ] **P1 reliability: storage retry/timeout policy**
  - add retry/backoff + explicit timeout budgets for S3/GCS operations
...
- [ ] **P2 hardening/perf/security follow-ups**
  - stream GCS uploads (avoid full-file memory reads)
  - remove dynamic atom creation from external config normalization paths
  - optimize row-size estimation + parquet serialization hot paths
  - add temp-file reaper and secret-handling improvements for persisted connection info
...
- [x] **Test harness foundations**
  ...
  - [x] optional provider-backed runtime snapshot-handoff recovery checks (pending -> explicit resume -> WAL continuation; S3-compatible + GCS, env-gated)
...
- [ ] **P0 data-integrity completion**
  - ack-after-durable-commit semantics (eliminate ack-before-commit loss window)
  - safe bootstrap/start-LSN policy for existing replication slots (no skip-forward)
- [ ] **Full integration suite** (Postgres + S3 + GCS + metadata DB)
  - expand provider-backed failure/reconcile matrix (timeouts/retries/recovery cleanup)
- [ ] **Replication client hardening (phase 2)**
  - bounded in-flight processing/backpressure from service batch execution path
  - reconnect policy + alerting policy finalization
- [ ] **Advanced recovery/reconciler loop**
  - reconcile `:encoded` batches by default
  - orphan detection, policy tuning, large-scale cleanup safety
- [ ] **Storage/network resiliency**
  - retry/backoff + timeout budgets for S3/GCS operations
  - stream GCS uploads for large-file safety
- [ ] **Parquet writer hardening** (type fidelity, performance tuning, and compatibility validation)
- [ ] **Security/ops hardening**
  - remove dynamic atom creation from external config normalization
  - improve persisted credential handling + temp-file reaper strategy
  - add telemetry for ack-vs-checkpoint lag / queue pressure / orphan indicators
...
1. **P0: eliminate ack-before-commit loss window**
   - implement durable-commit-driven ack advancement (service/meta checkpoint -> connection ack state)
   - add crash-injection coverage at each handoff boundary to prove no silent loss

2. **P0: fix bootstrap/start-LSN behavior for existing slots**
   - replace skip-forward behavior with backlog-safe slot/checkpoint resolution rules
   - add restart/backlog integration tests for slot already-existing cases

3. **P0/P1: close stranded-state recovery gaps**
   - reconcile `:encoded` by default (or remove exposed intermediate state)
   - add explicit crash-window tests for upload recorded/not-recorded edge cases

4. **P1: bounded runtime backpressure path**
   - move long write/upload/commit work off the service mailbox hot path
   - add bounded in-flight controls + telemetry/alerts for queue pressure and lag

5. **P1: storage resiliency policy**
   - add explicit retry/backoff + timeout budgets for S3/GCS operations
   - implement streaming GCS uploads for large file safety

6. **Provider-backed failure matrix expansion**
   - keep local filesystem-backed integration as primary gate
   - extend provider-backed tests into transient failure, retry, reconcile, and restart semantics

7. **Parquet + perf/security hardening**
   - parquet typing/perf compatibility improvements
   - remove dynamic atom creation from external config paths
   - temp-file reaper + credential-handling hardening + richer failure telemetry

8. **DuckLake metadata maturation (phase 2, deep-parity)**
...
9. **Dependency footprint minimization (Elixir + Rust)**
